...
tables = c("sites","datasets","collectionunits","datasetdatabases","constituentdatabases","datasettypes","contacts","datasetpis")
...
vert_datasets = datasets_df %>% left_join(datasettypes_df, by=join_by(datasettypeid)) %>% left_join(datasetdatabases_df,by=join_by(datasetid)) %>% dplyr::filter(datasettype %in% c("vertebrate fauna","specimen stable isotope")) %>% left_join(collectionunits_df, by=join_by(collectionunitid)) %>% left_join(sites_df, by=join_by(siteid)) %>% left_join(constituentdatabases_df,by=join_by(databaseid)) %>% left_join(datasetpis_df,by=join_by(datasetid)) %>% left_join(contacts_df,by=join_by(contactid.y == contactid))
...
pal1 <- colorFactor(palette = "Set1", domain = point_vert_datasets$databasename)
pal2 <- colorFactor(palette = "Set1", domain = poly_vert_datasets$databasename)
...
latin_verts = point_vert_datasets %>% st_filter(coordinates_sf) %>% st_filter(coordinates_sf2, .predicate=st_disjoint) %>% select(datasetid,siteid,datasettype,sitename,databasename,contactname,link)

latin_verts2 = poly_vert_datasets %>% st_filter(coordinates_sf) %>% st_filter(coordinates_sf2, .predicate=st_disjoint) %>% select(datasetid,siteid,datasettype,sitename,databasename,contactname,link)

latin_verts_all = latin_verts %>% rbind(latin_verts2) %>% select(!link)


pal3 <- colorFactor(palette = "Set1", domain = latin_verts_all$databasename)
...
leaflet() %>%
  ...
  addPolygons(data=latin_verts2,
              fillColor=~pal3(databasename),
              color=~pal3(databasename),
              weight=2,
              opacity=0.5,
              fillOpacity=0.5,
              popup = ~link) %>%
  ...
  addLegend("bottomright", 
            pal = pal3, 
            values = latin_verts_all$databasename,
            title = "Database Name",
            opacity = 1)
...
med_verts = point_vert_datasets %>% st_filter(coordinates_sf3) %>% select(datasetid,siteid,datasettype,sitename,databasename,contactname,link)

med_verts2 = poly_vert_datasets %>% st_filter(coordinates_sf3) %>% select(datasetid,siteid,datasettype,sitename,databasename,contactname,link)

med_verts_all = med_verts %>% rbind(med_verts2) %>% select(!link)



pal4 <- colorFactor(palette = "Set1", domain = med_verts_all$databasename)
...
leaflet() %>%
  ...
  addPolygons(data=med_verts2,
              fillColor=~pal4(databasename),
              color=~pal4(databasename),
              weight=2,
              opacity=0.5,
              fillOpacity=0.5,
              popup = ~link) %>%
  ...
  addLegend("bottomright", 
            pal = pal4, 
            values = med_verts_all$databasename,
            title = "Database Name",
            opacity = 1)
...
point_vert_neo = point_vert_neo %>% select(datasetid,siteid,datasettype,sitename,databasename,contactname,link)

poly_vert_neo = poly_vert_neo %>% select(datasetid,siteid,datasettype,sitename,databasename,contactname,link)


neo_all = point_vert_neo %>% rbind(poly_vert_neo) %>% select(!link)
