...
dois = dois_a %>% dplyr::filter(!doi  %in% c("10.1073\002pnas.0530193100","10.1073\002pnas.0408315102"))


dois = dois %>% dplyr::filter(!doi %in% c("10.1002/(SICI)1099-1417(199709/10)12:5<347::AID-JQS319>3.0.CO;2-#","10.1130/0091-7613(1987) 15<837:CRDOTP> 2.0.CO;2","10.1016","10.1111"))
...
fuzzes = list()
whichfuzz_neo = list()
terms_list = c("sensitive site","geomask","low-resolution map","fuzzed site", "obscured location", "purposeful imprecision","obfuscate", "county-level")
...
permits = list()
which_per = list()
terms_list_per = c("permit number","permit #")
...
for ( i in seq(length(human_rcs))) {
  ...
  for (j in seq(length(human_rcs[[i]]$highlight))) {
    ...
    human_rc_mat[idx,11] = human_rcs[[i]]$highlight[[j]]}
