...
I also created four csv files for Kitty (below). 

"Publication Authors" lists every publication that Neotoma knows about concerning vertebrate fauna datasets from Florida, along with the relevant datasets and authors. Since a publication can concern multiple datasets and have multiple authors, a particular publication will often show up multiple times in this table. If, for instance, a publication has three authors and concerns two datasets, there will actually be six rows in the table for that publication, one for each combination of dataset and author.
...
"Harmonization Table" lists the corresponding Neotoma name for every site from Kitty's excel file. The only two sites without definite matches from Kitty's file were Vero 2 and Boynton. Vero 2 likely corresponds to Vero, based on Kitty's note that in the VP archives the site is "called Vero (General Area) and not under Vero 2". There are two vertebrate datasets in Boynton Mound Complex site in Neotoma, so maybe one of those datasets corresponds to the Boyton Mound Complex site and the other to the site labeled Boynton?

"Unaccounted Sites" lists all the vertebrate fauna sites in Neotoma that don't have any corresponding sites in the FaunMap excel sheet that Kitty sent over. 
...
<ul class="downloads">
          ...
          <li><a href="unaccountedNeoSites.csv">Download <strong>Unaccounted Sites</strong></a></li>
        </ul>
...
repospec_FL = repospec_df %>%
  dplyr::filter(datasetid %in% data_id$datasetid)
