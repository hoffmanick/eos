...
I also created two csv files for Kitty (below). "Publication Authors" lists every publication that Neotoma knows about concerning vertebrate fauna datasets from Florida, along with the relevant datasets and authors. Since a publication can concern multiple datasets and have multiple authors, a particular publication will often show up multiple times in this table. If, for instance, a publication has three authors and concerns two datasets, there will actually be six rows in the table for that publication, one for each combination of dataset and author.
