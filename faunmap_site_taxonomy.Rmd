...
```{r join,missing=FALSE,echo=FALSE,warning=FALSE}
...
sitename_results_abb1 = sitename_results %>% dplyr::select(siteid,sitename) %>% rbind(archaeo_faun_abb) %>% distinct(siteid, sitename)

sitename_results_abb = sitename_results_abb1 %>% dplyr::mutate(faun_name = str_replace(sitename, "\\s*\\[.*$", "")) %>% left_join(locs, by=join_by('faun_name' == 'sitename_f')) %>% dplyr::filter(sitenumber == "" | is.na(sitenumber) | (sitenumber ==str_extract(sitename, "(?<=\\[).*?(?=\\])") ))

addendum_sites1 = sitename_results_abb1 %>% dplyr::filter(! sitename %in% sitename_results_abb$sitename) %>% left_join(locs, by=join_by('sitename' == 'sitename_f')) %>% dplyr::filter(!is.na(locid))

addendum_sites2 = sitename_results_abb1 %>% dplyr::filter(! sitename %in% sitename_results_abb$sitename) %>%left_join(locs, by=join_by('sitename' == 'sitename_f')) %>% dplyr::filter(is.na(locid))  %>% dplyr::mutate(faun_name = str_replace(sitename, "\\s*\\[.*$", "")) %>% dplyr::select(siteid,sitename,faun_name) %>% left_join(locs, by=join_by('faun_name' == 'sitename_f')) %>% dplyr::mutate(extracted = str_extract(sitename,  "(?:(?:\\[[^\\]]*\\]){2}|\\[[^\\]]*\\])$")) %>% dplyr::mutate(extracted = str_replace_all(extracted,  "[\\[\\]]", "")) %>% group_by(sitename) %>% dplyr::filter(sitenumber == extracted | n() == 1 )


sitename_joined = sitename_results_abb %>% dplyr::select(siteid,sitename,sitenumber,geographic_precision)

addendum_joined = addendum_sites1 %>% dplyr::select(siteid,sitename,sitenumber,geographic_precision) %>% rbind(sitename_joined) 

addendum_joined2 = addendum_sites2 %>% dplyr::select(siteid,sitename,sitenumber,geographic_precision) %>% rbind(addendum_joined) %>% distinct(siteid,.keep_all=TRUE)
...
dinaa = content(GET('https://opencontext.org/query/Americas/United+States.json?response=metadata,geo-reco[…]ite&proj=52-digital-index-of-north-american-archaeology-dinaa'))
