...
tables = c("datasets","publications", "datasetpublications","collectionunits","datasetdatabases","constituentdatabases","sites","depenvttypes", "variables")
...
faun_dats = matrix(nrow=idx,ncol=7)
...
names(faun_dat_df) = c("value","units","elementtype","variablename","datasetid","siteid","sitename")
...
faun_dt = faun_sites2 %>% select(siteid,sitename,faun_name,extracted,sitenumber,geographic_precision,sitename_sensitive,smith,archaeo) %>% left_join(unittypes)
...
`r numspecsites` sites in FAUNMAP have units associated with them that are more specific than presence absence. Here are counts of those units:


```{r specsitesshow,message=FALSE,warning=FALSE,echo=FALSE}

specific %>% group_by(siteid,sitename) %>% summarize(toString(unique(units))) %>% datatable(rownames=FALSE)
```
# Priorities
