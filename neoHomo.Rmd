...
## Methods
According to the Neotoma Manual, "The lat-long box can be used either to circumscribe the areal extent of a site or to provide purposeful imprecision to the site location." We therefore decided to count any bounding boxes for geography, rather than points, as fuzzed sites. Notice that this is a liberal definition - some sites with bounding boxes will have been formatted that way for reasons other than purposeful imprecision. We found `r num_fuzzed` fuzzed sites. One table below documents the site names, dataset types, and constituent databases associated with fuzzed sites. The next table counts datasets associated with fuzzed sites by the type of dataset and the constituent database from which the dataset derives, and the map below documents fuzzed site locations.
