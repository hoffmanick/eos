...
My first strategy here is to get the full "repository specimens" table (which connects dataset ids to repository ids) and filter it just for those datasets connected to sites in Florida. This leads to 159 datasets in repositories. But it's unclear that there are actually specimens in these repositories...
...
The following table shows which institutions are the largest holders of Floridian paleoecological specimens (by strategy 1). 
...
```
Unfortunately, when I do a Tilia call to find the specimen IDs associated with these datasets, none show up - there are no specimens listed for these datasets, even though the datasets are connected to repositories in the repositoryspecimens table.


## Other strategy for specimens

My second strategy is to grab the entire specimens table (3351 observations) and get the sample id, collection unit id, analysis unit id, and site id associated with each specimen, and to filter that table for sites from Florida. However, in this case I find that there are no specimens from sites in Florida, of the 435 sites listed here. And only 3 of the datasets actually associated with specimens are held at Florida Museum of Natural History. These three datasets have 13 specimens associated with them.
...
One problem here is that the repository hosting the specimens according to the specimens table (specimen -> site -> dataset -> repository)is the Florida Museum of Natural History - but according to the repository specimens table there are some discrepancies. I think two of these datasets may be associated with multiple repositories, but only have specimens connected to them from the non-FLMNH repository.
...
tm_shape(bg_big)+
    ...
    tm_layout(legend.position=c("LEFT","BOTTOM"),
              ...
              legend.width=0.4,
              legend.text.size=0.5,
              main.title= 'Neotoma Spec sites: from Specimens', 
              main.title.position = "center",
              title.bg.color = "white", panel.label.height=1)
...
Then I try a third strategy. I grab the repository specimens and get all specimens associated with each dataset id through a Tilia call. There are 6404 rows in repository specimens, but only 112 of these datasets have specimens associated with them through the Tilia call. Of the 895 specimens connected to these 112 datasets, they are from 25 sites, none of which are in Florida. 9 of these specimens are held by the Florida Museum of Natural History.
...
One thing I'm realizing is that there are both specimens unconnected to datasets (2,456 out of 3,351), and datasets with connections to repositories that are unconnected to specimens (6,143 out of 6,255 distinct datasets in repositorydatasets).
