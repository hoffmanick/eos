...
My first strategy here is to get the full "repository specimens" table (which connects dataset ids to repository ids) and connect it to all the sites in Florida that have dataset ids listed in the table. This theoretically leads to a lot of representation in repositories. But it's unclear that there are actually specimens in these repositories...
...
My second strategy is to grab the entire specimens table (3351 observations) and get the sample id, collection unit id, analysis unit id, and site id associated with each specimen, and to use the site id to get information on sites through that. However, in this case I find that there are no specimens from sites in Florida, of the 435 sites listed here. And only 5 of these datasets are held at Florida Museum of Natural History.
...
datatable(fl_specs[c(5,6,13,18)],rownames = FALSE)
```
