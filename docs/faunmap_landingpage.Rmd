...
for (i in seq(1,length(indigTerr))) {
  ...
  full_single_pols = left_join(indigTerr_mat,place_sf)
  
  empties = full_single_pols %>% dplyr::filter(st_is_empty(geometry))
...
a = which(st_is_empty( full_single_pols$geometry))
  
empties = full_single_pols %>% dplyr::filter(st_is_empty( full_single_pols$geometry))
...
full_single_pols = full_single_pols %>% mutate(pol = 1)

indigTerr_sf = rbind(full_single_pols,mults) %>% st_as_sf(crs="+datum=WGS84")
...
tm_shape(osm.raster(indigTerr_ne)) + tm_rgb() +
  tm_shape(indigTerr_ne) + tm_polygons(alpha=0.5)
