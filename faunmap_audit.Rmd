...
One of Neotoma's constituent databases is FAUNMAP: an effort originally by vertebrate paleontologists Ernest Lundelius and Russell Graham to compile data from across the United States on mammal faunas of the Late Quaternary period for the purpose of generating paleobiological and paleoclimatic insights. By 1995, FAUNMAP consisted of over 2900 sites. In the 2000s, FAUNMAP merged with the Global Pollen Databases and others to form Neotoma. It is a living database which as of `r Sys.Date()` consists of `r num_sites` sites.
...
We searched programmatically through FAUNMAP’s metadata for any sensitive Indigenous data. Such sensitive Indigenous data include those that concern Indigenous ancestors (e.g., human skeletal elements and human-derived radiocarbon dates) or derive from sacred sites (e.g., burial mounds). We also searched for any records in which purposeful imprecision has been introduced into the site coordinates because introducing imprecision is one way to protect sensitive sites. Lastly, we searched for datasets that were collected from federally recognized Indigenous lands, since these may be most relevant to nation building activities of tribal governments.

We documented sensitive Indigenous records, fuzzed sites, and data from federally recognized Indigenous lands. Although limitations in FAUNMAP’s metadata led us to systematically undercount or overcount records in different circumstances, we are hopeful that this preliminary inventory will spur us to better steward these data.
...
* are associated with a Smithsonian trinomial, and
...
The following table offers information on the `r num_sites` sites in FAUNMAP associated with Smithsonian trinomials, and we map them below the table.
...
```
`r original_smiths` of these sites have Smithsonian trinomials associated with them. Here is a on overall breakdown of the precision of those sites:
...
sitename_results= faun_sites[matches_sitename,]
...
We search in the names of our sites for any which indicate a potential for sensitivity using the list of terms below. We find `r num_sacredsites` such sites.
...
# FAUNMAP's collection units

Another important element of Neotoma's datastructure is the collectionunits table. A single site can have multiple collection units, which may correspond to a sediment core, or in FAUNMAP's case, more likely an excavation.

We query FAUNMAP's collection units to

* search for archaeological depositional environments, and
* sacred sites.
...
Sensitive sites may be identified through their collection unit metadata. However, no collection units were found that had any of the below terms in their notes or location fields.
...
Below are the records of human ancestors in FAUNMAP. Note that many of FAUNMAP's human records have been shielded from public view as Neotoma reevaluates its access policy for sensitive data.
